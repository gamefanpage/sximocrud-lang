<div class="sbox ">
 ...
 {{ Form::open(array('url'=>'user/signin', 'class'=>'form-vertical')) }}
	...
	<p class="text-center"><a  id="or"  href="javascript://ajax"><small> {{ Lang::get('core.fr_forgetpassword'); }} </small></a></p>
	<p class="text-muted text-center"> {{ Lang::get('core.fr_noaccount'); }} </p>
				@if(CNF_REGIST =='true') 
		  		<a class="btn btn-default btn-white btn-white btn-block"  href="{{ URL::to('user/register')}}">  {{ Lang::get('core.fr_createaccount'); }}  </a>
			@endif	
	  ...
	  <a href="{{ URL::to('')}}"> {{ Lang::get('core.fr_backsite') }} </a>  
   		</p>
   	</div>				
	</div>		  
